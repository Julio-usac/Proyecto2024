...
function BienesFungibles() {
 ...
 //Funcion utilizada para llamar al endpoint DescargarBitacora y descargar la bitacora
  ...
  const Descargar = async() => {
    if(fecha && fecha2){
      try {
        const response = await axios.get(url+'/DescargarBienesFungibles/', { responseType: 'blob',
        params: {
          fecha1: fecha,
          fecha2: fecha2,
          usuario: userid
  ...
  const DescargarPDF = async() => {
    if(fecha && fecha2){
      try {
        const response = await axios.get(url+'/ReportePDFbienesFungibles/',
        { 
          responseType: 'blob',
          params: {
            fecha1: fecha,
  ...
  const ObtenerBienes= async() => {
    if(fecha && fecha2){
      try {
        const response = await axios.get(url+'/BienesFungibles/', { params: {
          fecha1: fecha,
          fecha2: fecha2
        },
        headers: {
  ...
  return (
    <AppLayout>
      <h1 className="text-5xl mt-6">Bienes Fungibles</h1>
...
export default BienesFungibles;
